...
import com.barikoi.cnlapp.databinding.FragmentLowStockProductBinding
...
class LowStockProductFragment : Fragment() {
    private lateinit var binding: FragmentLowStockProductBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentLowStockProductBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun init() {
        ...
        getSummaryProducts(Api.lowstock_product_list + "?user_id=" + userId)
    }
            ...
            override fun onResponseSuccess(response: String) {
                try {
                    ...
                    if (response != null) {
                        ...
                        val productsArray = obj.getJSONArray("low-stock")
                        if (productsArray.length() > 0) {
                            for (i in 0 until productsArray.length()) {
                                val productObj = productsArray.getJSONObject(i)
                                if (!productObj.isNull("current_available_stock")) {
                                    if (!productObj.isNull("products")) {
                                        val productName = productObj.getJSONObject("products")
