...
class BouncedOrderFragment : Fragment(), OrderListSuccessListener, OnEditOrderListener {
        ...
        etSearchShop!!.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                if (s!!.isEmpty()) {
                    if (sr_id!!.isEmpty()) {
                        getAllOrders(
                            Api.get_saved_order + "?user_id=" + user_id + "&start_date=" + StartDate + " 00:00:00" + "&end_date=" + EndDate + " 23:59:59" + "&territory_id=" + territory_id + "&order_status=DELIVERED,CANCELLED&include_filter_by_user_id=1",
                            queue!!,
                            token!!,
                            mCallback3!!
                        )
                        ...
                        getAllOrders(
                            Api.get_saved_order + "?user_id=" + user_id + "&start_date=" + StartDate + " 00:00:00" + "&end_date=" + EndDate + " 23:59:59" + "&order_status=DELIVERED,CANCELLED",
                            queue!!,
                            token!!,
                            mCallback3!!
                        )
            ...
            if (mCallback3 != null) {
                if (sr_id.isEmpty()) {
                    if (sharePrefUtils.getString(Api.USER_TYPE).equals("ASM")) {
                        getAllOrders(
                            Api.get_saved_order + "?user_id=" + user_id + "&start_date=" + start + " 00:00:00" + "&end_date=" + end + " 23:59:59" + "&territory_id=" + territory_id + "&order_status=DELIVERED,CANCELLED&region_id=${regionId}&include_filter_by_asm=1",
                            queue,
                            token,
                            mCallback3!!
                        )
                        ...
                        getAllOrders(
                            Api.get_saved_order + "?user_id=" + user_id + "&start_date=" + start + " 00:00:00" + "&end_date=" + end + " 23:59:59" + "&territory_id=" + territory_id + "&order_status=DELIVERED,CANCELLED&include_filter_by_user_id=1",
                            queue,
                            token,
                            mCallback3!!
                        )
    ...
    override fun onSuccess(array: JSONArray) {
        ...
        try {
            if (array.length() > 0) {
                for (i in 0 until array.length()) {
                    ...
                    if (orderObj.getString("order_status")
                            .equals("DELIVERED", true) || orderObj.getString("order_status")
                        ...
                        if (brandArray.length() > 0) {
                            for (j in 0 until brandArray.length()) {
                                ...
                                if (orderObj.getString("order_status").equals(
                                        "DELIVERED",
                                        true
                                    ) && brandObj.getInt("bounced_quantity") > 0
                                ) {
                                    if (bounce > 0) {
                                        productItems.add(
                                            Products(
            ...
            recylerView.apply {
                adapter = OrderDeliveryListAdapter(
                    orderList,
                    listener!!,
            ...
            ViewUtils.viewDialog(mContext,
                ...
                object :
                    ...
                    override fun onConfirmed() {
                        val status = itemValue[isChecked].uppercase(Locale.ENGLISH)
