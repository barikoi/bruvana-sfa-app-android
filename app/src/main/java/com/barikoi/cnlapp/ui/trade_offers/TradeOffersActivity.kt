...
class TradeOffersActivity : BaseActivity() {
    private lateinit var binding: ActivityTradeOffersBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.filterLayout.setOnClickListener {
            val popup = PopupMenu(applicationContext, binding.filterTitle)
                ...
                override fun onMenuItemClick(item: MenuItem): Boolean {
                    when (item.itemId) {
                        R.id.menu_all_product -> {
                            if (itemList.size > 0) {
                                if (adapter != null) {
                                    ...
                                    binding.productList2.layoutManager = StaggeredGridLayoutManager(
                                        2,
                                        StaggeredGridLayoutManager.VERTICAL
                                    )
                                    binding.productList2.adapter = adapter
                                    adapter!!.notifyDataSetChanged()
                                }

                            }
                            binding.tvTitle.setText(resources.getString(R.string.product_list))
                        }
                        ...
                        R.id.menu_trade_offers -> {
                            ...
                            binding.productList2.layoutManager =
                                StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL)
                            binding.productList2.adapter = adapter
                            adapter!!.notifyDataSetChanged()
                            binding.tvTitle.setText(resources.getString(R.string.title_trade_offers))
                        }
                    }
                    return true
                }
                            ...
                            for (i in 0 until productsArray.length()) {
                                ...
                                if (productObj.has("trade") && !productObj.isNull("trade")) {
                                    if (productObj.getJSONArray("trade").length() > 0) {
                                        tradeProducts.add(
                                            TradeProduct(
                                                productObj.getJSONArray("trade").getJSONObject(0)
                                                    .getString("start_date"),
                                                productObj.getJSONArray("trade").getJSONObject(0)
                                                    .getString("end_date"),
                                                productObj.getJSONArray("trade").getJSONObject(0)
                                                    .getString("discounted_price")
                                            )
                                        )
                                    }
                                }
