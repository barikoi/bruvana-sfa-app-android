...
class CreateShopActivity : AppCompatActivity(), OnMapReadyCallback, PermissionsListener {
    ...
    private var selectedMarket: String? = ""
    ...
    var marketsList : ArrayList<String>? = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        spinnerLayoutMarkets.setBackgroundDrawable(gd)
        ...
        routeNameList = intent.getParcelableArrayListExtra<Parcelable>("routeList") as ArrayList<Pair<String, String>>
        /*marketsList = intent.getStringArrayListExtra("markets")
        marketNameList = intent.getParcelableArrayListExtra<Parcelable>("marketList") as ArrayList<Pair<String, Pair<String, String>>>*/
                                    ...
                                    for (i in 0 until routesArray.length()) {
                                        ...
                                        marketNameList!!.add(Pair(routeObj.getString("route_name"),
                                            Pair(
                                                ...
                                                routeObj.getString("market_name")
                                            )
                                        )
                                        )
                                        marketsList!!.add(routeObj.getString("market_name"))
                                    }

                                    if (marketsList!!.size > 0){
                                        val exists = routesList!!.find {
                                            it == resources.getString(R.string.select_route)
                                        }
            ...
            spinnerRoutes.onItemSelectedListener =
                object : AdapterView.OnItemSelectedListener {
                    override fun onItemSelected(
                        ...
                        markets!!.add(0, resources.getString(R.string.select_market))
                        marketPairList!!.add(0, Pair("",Pair(
                            "",
                            resources.getString(R.string.select_market)
                        )))
                        for (i in 0 until marketNameList!!.size) {
                            if (marketNameList!![i].first == routesList[p2]) {
                                markets.add(marketNameList!![i].second.second)
                                marketPairList.add(Pair(routesList[p2], Pair(marketNameList!![i].second.first, marketNameList!![i].second.second)))
                            }

                        }
                        if (markets.size > 0) {
                            setMarkets(markets,marketPairList)
                        }
                    }
    ...
    fun setMarkets(marketList: ArrayList<String>, marketNameList: ArrayList<Pair<String, Pair<String, String>>>){
        ...
        if (spinnerMarkets != null) {
            ...
            spinnerMarkets.onItemSelectedListener =
                object : AdapterView.OnItemSelectedListener {
                    override fun onItemSelected(
                        p0: AdapterView<*>?,
                        p1: View?,
            ...
            if (shops != null){
                if(shops!!.market_name.length>0){
                    val pos= (spinnerMarkets.adapter as ArrayAdapter<String>).getPosition(shops!!.market_name)
                    if(pos>-1) {
                        spinnerMarkets.setSelection(pos)
                    }
                }
    ...
    fun createShop() {
        ...
        if (selectedMarket!!.length == 0) {
            inputOk = false
            Toast.makeText(applicationContext, "Need to select Market", Toast.LENGTH_LONG).show()
            hideProgress(progressBarShop)
        }
